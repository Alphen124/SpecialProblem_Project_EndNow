(function () {
  ...
  function inferSource(item: CartItem): string {
    if (item?.source) return item.source;
    const price = item?.price;
    const total = item?.total;
    if ((price === '' || price == null) && typeof total === 'number' && total === 0) return 'borrow';
    return 'rent';
  }

  function addCartToHistory(items: CartItem[]): void {
    const history = getJson<CartItem[]>(HISTORY_KEY, []);
  ...
  function buildCartMeta(it: CartItem): string {
    try {
      const dates = String(it?.date ?? '').split(' - ').map(s => s.trim()).filter(Boolean);
      if (dates.length === 1) return `${dates[0]} ${it?.pickup ?? ''} - ${it?.returnt ?? ''}`.trim();
      if (dates.length >= 2) return `${dates[0]} ${it?.pickup ?? ''} - ${dates[1]} ${it?.returnt ?? ''}`.trim();
  ...
  function renderMiniCart(): void {
    const itemsWrap = document.getElementById('miniCartItems');
    const totalEl = document.getElementById('miniCartTotal');
    if (!itemsWrap) return;

    const cart = getJson<CartItem[]>(CART_KEY, []);
    itemsWrap.innerHTML = '';
    ...
    if (totalEl) totalEl.textContent = hasPriced ? 'Total ' + total.toFixed(2) + ' Baht' : '';

    itemsWrap.querySelectorAll('.mini-delete').forEach(btn => {
      btn.addEventListener('click', () => {
        const i = parseInt((btn as HTMLElement).getAttribute('data-idx') ?? '-1', 10);
        const cart = getJson<CartItem[]>(CART_KEY, []);
  ...
  function renderMiniFav(): void {
    ...
    if (countEl) countEl.textContent = favs.length + ' favorites';
    ...
    itemsWrap.querySelectorAll('.mini-fav-delete').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (isNaN(i) || i < 0 || i >= favs.length) return;
        favs.splice(i, 1);
        setJson(FAV_KEY, favs);
        renderMiniFav();
        if (favs.length === 0) showMiniFav(false);
      });
    });
  }

  function showPopupAtAnchor(
    popupId: string,
    anchorId: string,
    show: boolean,
    renderFn?: () => void
  ...
  function showMiniCart(show: boolean): void {
    showPopupAtAnchor('miniCartPopup', 'headerCartBtn', show, renderMiniCart);
  }

  function showMiniFav(show: boolean): void {
  ...
  function wire(): void {
    ...
    function reposition(): void {
      try {
        const cartPopup = document.getElementById('miniCartPopup');
        if (cartPopup && !cartPopup.classList.contains('hidden')) showMiniCart(true);
        const favPopup = document.getElementById('miniFavPopup');
        if (favPopup && !favPopup.classList.contains('hidden')) showMiniFav(true);
      } catch { /* ignore */ }
    }
    ...
    document.getElementById('confirmYes')?.addEventListener('click', () => {
      ...
      renderMiniCart();
      showMiniCart(false);
      if (!hasBorrow) alert('Checkout complete (demo)');
    });
  }
...
(function () {
  function setupRentoutLink(): void {
    document.querySelectorAll<HTMLAnchorElement>('.nav a[href="/features/devices/rentout.html"]').forEach(link => {
...
(function () {
  function setupDevicesLink(): void {
    document
      .querySelectorAll<HTMLAnchorElement>(
        '.nav a[href="/features/devices/devices.html"], .nav a[href="/features/auth/devices-auth.html"]'
      )
      .forEach(link => {
        link.addEventListener('click', function (e: MouseEvent) {
          e.preventDefault();
