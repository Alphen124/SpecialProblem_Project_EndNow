...
function saveAuthData(accessToken: string, refreshToken: string, user: User): void {
...
async function login(email: string, password: string): Promise<{ success: boolean; user: User }> {
  const data = await apiRequest<{ access_token: string; refresh_token: string; user: User }>(
    '/api/auth/login',
    {
      method: 'POST',
      skipAuth: true,
...
async function getProfile(): Promise<User> {
  const data = await apiRequest<User>('/api/auth/profile', { method: 'GET' });
...
function updateHeaderForAuth(): void {
  ...
  if (isAuthenticated()) {
    ...
    headerActions.innerHTML = `
      <a href="/features/auth/login.html" class="btn btn-primary">เข้าสู่ระบบ</a>
      <a href="/features/auth/register.html" class="btn btn-outline">สมัครสมาชิก</a>
    `;
  }
}
...
function closeUserMenuOnClick(event: MouseEvent): void {
  const popup = document.getElementById('userPopupCard');
  const btn = document.getElementById('userMenuBtn');
  if (popup && !popup.classList.contains('hidden')) {
    const target = event.target as Node;
    if (!popup.contains(target) && btn && !btn.contains(target)) {
...
const NoteLetAuth: NoteLetAuthNamespace = {
  isAuthenticated,
  requireAuth(): boolean {
    if (!isAuthenticated()) {
      window.location.href = '/features/auth/login.html';
...
document.addEventListener('click', function (event: MouseEvent) {
  ...
  if (userMenu && dropdown && !userMenu.contains(event.target as Node)) {
