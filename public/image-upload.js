...
(function() {
  ...
  if (addImageInput && addImagePreview) {
    addImageInput.addEventListener('change', (e) => {
      handleImagePreview(e.target.files, addImagePreview);
    });
  }
  ...
  if (editImageInput && editImagePreview) {
    editImageInput.addEventListener('change', (e) => {
      handleImagePreview(e.target.files, editImagePreview);
    });
  }
  ...
  function handleImagePreview(files, previewContainer) {
    ...
    filesToPreview.forEach((file, index) => {
      ...
      reader.onload = (e) => {
        ...
        previewItem.style.cssText = 'position: relative; aspect-ratio: 1; border: 2px solid #ddd; border-radius: 8px; overflow: hidden;';
        ...
        removeBtn.style.cssText = 'position: absolute; top: 4px; right: 4px; background: rgba(0,0,0,0.7); color: white; border: none; border-radius: 50%; width: 24px; height: 24px; cursor: pointer; font-size: 18px; line-height: 1; padding: 0;';
